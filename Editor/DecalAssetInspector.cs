...
namespace SamDriver.Decal
{
    ...
    public class DecalAssetInspector : Editor
    {
        ...
        void PreviewAndRegionSelectionGUI()
        {
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                using (new EditorGUILayout.VerticalScope(GUILayout.Width(nudgeButtonSize)))
                {
                    ...
                    Vector4 bounds = ((DecalAsset) target).BoundsAsVector4;

                    EditorGUI.DrawPreviewTexture(previewRegion, previewTexture);

                    // TODO: Should be able to use `overlayBoundsMaterial` as custom material but
                    // it keeps failing. So drawing rectangles over basic image instead.
                    var fillColour = new Color(0f, 0f, 0f, 0.75f);

                    // Area being used by this decal.
                    float x = Mathf.Round(bounds.x * previewRegion.width);
                    float y = Mathf.Round((1f - bounds.w) * previewRegion.height);
                    float w = Mathf.Round((bounds.z - bounds.x) * previewRegion.width);
                    float h = Mathf.Round((bounds.w - bounds.y) * previewRegion.height);
        ...
        int width, int height)
        {
            ...
            thumbnailMaterial.SetColor("_ColourForAlpha", previewBackColour);
